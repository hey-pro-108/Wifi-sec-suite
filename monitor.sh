...
start_monitor() {
    ...
    while true; do
        ...
        if [ -n "$current_arp" ] && [ "$current_arp" != "$arp_cache" ]; then
            local new_entries=$(comm -13 <(echo "$arp_cache" | sort) <(echo "$current_arp" | sort) 2>/dev/null)
            if [ -n "$new_entries" ]; then
                ...
                echo "$new_entries" | while read -r line; do
                    ...
                    if [ -n "$ip" ] && [ "$ip" != "$gateway" ] && [ "$ip" != "$local_ip" ]; then
                        echo "      IP: $ip | MAC: $mac"
                        ...
                        if [ "$ip" = "$gateway" ] && [ -n "$arp_cache" ]; then
                            echo "[MITM] Possible ARP spoofing detected! Gateway MAC changed"
                            log_threat "ARP_SPOOF" "$ip" "Gateway MAC changed"
                        fi
                    fi
                done
                echo ""
